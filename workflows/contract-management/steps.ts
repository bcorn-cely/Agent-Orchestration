...
import { generateContractDraft as generateContractDraftAgent } from '@/workflows/contract-management/agent/subagents/contract-generation-agent';
import { extractStructuredData as extractStructuredDataAgent } from '@/workflows/contract-management/agent/subagents/extraction-agent';
import { validateClauses as validateClausesAgent } from '@/workflows/contract-management/agent/subagents/clause-validation-agent';
import { detectRisks as detectRisksAgent } from '@/workflows/contract-management/agent/subagents/risk-detection-agent';
...
const POLICIES = process.env.POLICIES ?? `${BASE}/api/mocks/contract-management/contracts/policies`;
...
export async function generateRedline(input: {
  ...
  const res = await fetch(`${BASE}/api/mocks/contract-management/contracts/redline`, {
    method: 'POST',
    body: JSON.stringify(input),
    headers: { 'content-type': 'application/json' },
  });
...
export async function sendApprovalRequest(
  ...
  const approvalUrl = `${BASE}/approve?token=${encodeURIComponent(token)}`;
...
Approval URL: ${url}`;
  ...
  await fetch(`${NOTIFY}/email`, {
    ...
    body: JSON.stringify({
      to: email,
      subject: `Contract Approval Required - ${role} Review - ${details.contractId || ''}`,
      body: emailBody,
      url,
...
export async function archiveContract(input: {
  ...
  const res = await fetch(`${BASE}/api/mocks/contract-management/contracts/archive`, {
    method: 'POST',
    body: JSON.stringify(input),
    headers: { 'content-type': 'application/json' },
  });
