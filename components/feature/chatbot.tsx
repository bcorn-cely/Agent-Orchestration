 ...
 * 
 * Client-side chat interface for the contract management system.
 * 
 * Key Features:
...
export function ContractChatbot({ id, initialMessages }: { id: string; initialMessages?: UIMessage[] }) {
  ...
  const { messages, sendMessage, status, addToolApprovalResponse } = useChat({
    ...
    transport: new DefaultChatTransport({
      api: "/api/chat/contract-management/contracts",
            ...
            <div className="px-6 pb-5 pt-4 border-b border-blue-500/20 bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 text-white rounded-t-xl" style={{ minHeight: 'fit-content' }}>
              <div className="flex items-center justify-between" style={{ contain: 'layout style' }}>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-lg">Contract Management AI</h3>
                </div>
            ...
            <div ref={messagesRef} className="flex-1 overflow-y-auto p-6 space-y-4 scroll-smooth bg-card">
              {messages.length === 0 && (
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-base font-medium text-foreground mb-2">Welcome to Contract Management</p>
                  ...
                  {message.role === "assistant" &&
                    message.parts
                      ...
                      .map((part: any, i: number) => {
                        ...
                        // The tool call ID might be in different fields depending on AI SDK version
                        // Try multiple possible fields to find the correct tool call ID
                        // The error shows IDs like "tooluse_J-pyZR4bTjae8dxXTtSFvw" so check all possible fields
                        ...
                        // Debug: log the part structure to understand the ID format
                        if (toolName === 'contractDraft' && part.state === 'approval-requested') {
                          console.log('[Chatbot] ContractDraft part structure:', {
                            part,
                            toolCallId,
